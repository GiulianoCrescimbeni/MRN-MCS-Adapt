...
void initialize_ues_if_needed(){
    ...
    for (int ue=0;ue<CONNECTED_UES;ue++){
        ...
        connected_ue_list[ue].mcs_dl = 10 + (rand() % 19);
    }
    is_initialized = true;
}
...
void apply_properties_to_ue_list(UeListM* ue_list){
    ...
    for(int ue=0; ue<ue_list->n_ue_info; ue++){
        ...
        set_ue_properties(ue_list->ue_info[ue]->rnti,
                          ue_list->ue_info[ue]->mcs_dl);
    }
}

void set_ue_properties(int rnti, int mcs_dl){
    ...
    for(int ue=0; ue<CONNECTED_UES; ue++) {
        if(connected_ue_list[ue].rnti == rnti){
            printf("RNTI found\n");
            connected_ue_list[ue].mcs_dl = mcs_dl;
...
UeListM* build_ue_list_message(){
    ...
    for(int i = 0; i<CONNECTED_UES; i++){
        ...
        ue_info_list[i]->has_ber_dl = 1;
        // Bit error rate as percentage
        ue_info_list[i]->ber_dl = (float)rand() / (float)RAND_MAX;

        ue_info_list[i]->has_mcs_dl = 1;
        ue_info_list[i]->mcs_dl = connected_ue_list[i].mcs_dl;
    }
